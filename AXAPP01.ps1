...
$parameters = @" 
AcceptLicenseTerms=1 HideUI=1 ConfigurePrerequisites=1 OptInCEIP=0 UseMicrosoftUpdate=1 InstallDatabase=1 DbSqlServer=$dbSqlServer DbSqlDatabaseName=MicrosoftDynamicsAx DbSqlModelStoreBaseline=MicrosoftDynamicsAxBaseline AdditionalModelFiles=$AdditionalModelFiles InstallAos=1 AosInstanceName=MicrosoftDynamicsAx AosPort=2712 AosWsdlPort=8101 AosNetTcpPort=8201 AosStart=1 AosAccount=$aosAccount AosAccountPassword=$aosAccountPassword InstallClientUI=1 ClientConfig=1 ClientLanguage=en-US ClientAosServer=$clientAosServer ClientInstallType=1 CreateClientDesktopShortcut=1 InstallNetBusinessConnector=1 BusinessConnectorProxyAccount=$aosAccount BusinessConnectorProxyAccountPassword=$aosAccountPassword InstallAifWebServices=1 AifWebServicesWebSite="Default Web Site" AifWebServicesApplicationPool=MicrosoftDynamicsAXAif60 AifWebServicesVirtualDirectory=MicrosoftDynamicsAXAif60 AifWebServicesAosAccounts=$aosAccount InstallOfficeAddins=1 InstallDebugger=1 InstallVisualStudioTools2013=1 InstallManagementUtilities=1
"@
